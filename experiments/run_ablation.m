function results = run_ablation(varargin)
    ...
    for v_idx = 1:size(variants, 1)
        ...
        fprintf('  >> %s 骞冲潎缁撴灉:\n', variant_desc);
        fprintf('     骞冲潎閫傚簲搴? %.2f +/- %.2f\n', mean(best_fits), std(best_fits));
        fprintf('     骞冲潎鑳借€? %.2f J\n', mean(energies));
        fprintf('     骞冲潎楂樹紭鍏堢骇瑕嗙洊鐜? %.2f%%\n', mean(cov_high));
        fprintf('     骞冲潎鍏ㄥ眬瑕嗙洊鐜? %.2f%%\n', mean(cov_total));
        fprintf('     骞冲潎HV: %.4f +/- %.4f\n', mean(hv_values), std(hv_values));
        fprintf('     骞冲潎Spread: %.4f\n', mean(spread_values));
    end

    fprintf('\n========== 璁＄畻IGD (浣跨敤鍚堝苟Pareto鍓嶆部浣滀负鍙傝€? ==========\n');
    all_pareto_points = [];
    for v_idx = 1:size(variants, 1)
        for run = 1:n_runs
    ...
    fprintf('%-35s | %-12s | %-12s | %-12s\n', ...
        '鍙樹綋', 'HV(mean卤std)', 'IGD(mean)', 'Spread(mean)');
    fprintf('%s\n', repmat('-', 1, 75));
    for v_idx = 1:size(variants, 1)
        variant_name = variants{v_idx, 1};
        r = results.(variant_name);
