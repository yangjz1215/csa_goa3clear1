...
variant_order = {'proposed', 'no_subpop', 'no_goa_turn', 'no_goa_repulsion', 'no_pareto_leader', 'no_adaptive_weight'};
variant_labels_map = struct( ...
    'proposed', 'Proposed cSA-GOA', ...
    'no_subpop', 'w/o Multi-Subpop', ...
    'no_goa_turn', 'w/o GOA Turn', ...
    'no_goa_repulsion', 'w/o GOA Repulsion', ...
    'no_pareto_leader', 'w/o Dynamic Leader', ...
    'no_adaptive_weight', 'w/o Adaptive Weight');
variant_colors_map = struct( ...
    'proposed', [0.8500, 0.3250, 0.0980], ...
    'no_subpop', [0.4940, 0.1840, 0.5560], ...
    'no_goa_turn', [0.0000, 0.4470, 0.7410], ...
    'no_goa_repulsion', [0.9290, 0.6940, 0.1250], ...
    'no_pareto_leader', [0.4660, 0.6740, 0.1880], ...
    'no_adaptive_weight', [0.3010, 0.7450, 0.9330]);
...
%% ========== Figure: 3D Pareto (same convention as comparison_pareto_3D) ==========
% 对比脚本 plot_comparison_all：每个算法只画「档案点最多的那一轮」，轴为 Utility–Latency–Energy。
% 消融原先把 30 轮所有点堆在一起且轴顺序不同，导致严重重叠、难读；此处对齐对比图逻辑与坐标轴。
...
for v = 1:n_vars
    ...
    for run = 1:length(pfs)
        pf = pfs{run};
        if ~isempty(pf) && size(pf, 1) > max_size && size(pf, 2) >= 3
            max_size = size(pf, 1);
            best_run = run;
        end
    end
    ...
    scatter3(pf(:, 1), pf(:, 2), pf(:, 3), sz, ...
        'MarkerEdgeColor', colors(v, :), 'MarkerFaceColor', colors(v, :), ...
        'MarkerFaceAlpha', 0.58, 'DisplayName', labels{v});
end
